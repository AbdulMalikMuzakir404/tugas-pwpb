...
if (isset($_POST['simpanbarang'])) {
  ...
  $harga      = $conn->real_escape_string($_POST['harga']);
    ...
    $namabarang = validate(filter_var($namabarang, FILTER_SANITIZE_STRING));
    $harga      = validate(filter_var($harga, FILTER_SANITIZE_STRING));

    $sql   = "INSERT INTO barang VALUES (NULL, '$namabarang', '$harga')";
    $saved = mysqli_query($conn, $sql);

    if ($saved) {
    	header("Location: barang.php?success=Data Berhasil Dimasukan");
        exit();
    } else {
    	header("Location: barang.php?error=Data Yang Anda Masukan Error!");
        exit();
    }
